...
### X1 — Urdu (right-to-left) and English ✅
- **App language**: Settings → "App language" (English / اردو) switches right away and is saved on the account, so every device and the Android app open in that language. The sign-in and sign-up pages have a one-tap "اردو / English" switch (remembered in a cookie for a year).
- **Whole app in Urdu**: `lang/ur.json` holds ~1,850 translations of the chat app, settings, sign-in pages, dialogs, toasts, system messages and server messages. The pages stay written in English; for Urdu the browser swaps the text (and titles, placeholders, confirm dialogs) as soon as it appears, including chats and menus the app adds later. The dictionary is its own download (~41 KB gzip) and only Urdu users load it; the page stays hidden for that moment (never more than 4 s).
- **People's own words are never translated**: messages, names, captions, polls, link previews, quick replies and About.
- **Right-to-left layout**: `<html dir="rtl">`; the CSS now uses logical properties (margin-inline-start, inset-inline-end…), so the whole layout mirrors (chat list on the right, menus, bubbles, settings rows) while English looks exactly as before. Back/forward/send arrows flip; messages, names and typing boxes follow the direction of what was written; phone numbers, emails and passwords stay left-to-right; dates use Urdu month and day names; Urdu uses a Naskh font.
...
### Android app top bar fix (app 1.2) ✅
- **Bug:** in app 1.1 on many phones (Android 14 and older, WebView below 140) a plain white band sat above the header, the status bar icons were white on white, and the indigo header was a status bar taller than normal. Reproduced on an Android 14 emulator (WebView 113, 720 × 1612 like the reporter's phone).
- **Cause:** Capacitor's SystemBars plugin kept the page below the bars there, while the page's CSS still added room for the status bar from `env(safe-area-inset-top)` (`max(var, env)`).
- **App 1.2 (versionCode 3):** new `SystemInsets.java` draws the page behind the status and navigation bars on Android 8+ and sends the bar heights as `--safe-area-inset-top/bottom` before the first paint (`html[data-native-insets]`); side bars, camera cutouts and the keyboard shrink the page; Android 7 keeps the page between an indigo status bar and a black navigation bar. Capacitor's own insets handling is off. The slow-connection screen gets readable bar icons.
- **Web (also fixes app 1.1 after deploy):** edge-anchored parts use `--app-inset-top/bottom` (env() in browsers, the app's values in the app); page shells keep `--app-safe-top` (0 in browsers, so browsers look as before). App 1.1 pages below the bars get `html.has-bars-outside` before the first paint (layout head script), kept up to date by `resources/js/native/system-bars.js` (ignores the keyboard and split screen): normal header height and dark status icons on the plain bar. The white band itself only goes away with app 1.2.
- **Bar icons:** status and navigation bars are set separately: light icons on the indigo header, in dark theme and over calls, photos and status updates; dark navigation buttons on light pages (3-button navigation); landscape and tablet headers are light, so dark icons there.
- **Also fixed:** the admin top bar reserved the status bar twice; the group info header has the indigo strip under the status bar; chat list rows were 6px wider than the list (a horizontal scrollbar line at the bottom of the screen); the lightbox and settings footer leave room for a 3-button navigation bar.
- **Checked on the emulator:** app 1.2 sign-in, chat list, open chat with keyboard, settings, dark theme, 3-button navigation, landscape; app 1.1 with the new web code (chat list, open chat with keyboard). Tests: `resources/js/native/__tests__/system-bars.test.js` (9). PHPUnit 492, Vitest 247.

### App name & icon from the admin panel ✅
- **Admin → App settings → App name & icon** (`#brand`): app name (max 30), icon upload (PNG/JPG/WebP, at least 512 × 512) and icon background colour, with a live preview of the square and the circle phone icon. Saved in `app_settings.brand`; audit log `app.brand_updated`.
- **Right away on the server:** the name replaces APP_NAME everywhere (`BrandService::apply()` at boot and before queued jobs; the email sender name follows unless another one was set), and the icon is used as favicon, apple-touch icon, web app manifest icons (maskable too), push/browser notification icon and the logo on sign-in, legal, app and admin pages (`<x-brand-mark>`).
- **Icon files:** made with PHP GD in `storage/app/public/brand/{version}/`: web sizes 32/96/180/192/512, maskable 512, and Android launcher, round, adaptive foreground (108dp) and splash (288dp) for mdpi–xxxhdpi. A finished square icon is used full size; a see-through logo is placed on the background colour. Without GD the picture is used as it is.
- **Phones:** the home-screen name and icon are part of the APK. `php artisan app:android-brand --url=https://chat.hunario.com` (reads `/app-brand.json`) or without `--url` (this installation) writes the name into `strings.xml` and `capacitor.config.json` and the icons into `mipmap-*` (adaptive icon on the background colour, splash icon) before building the next APK.
- **Checked:** admin section in the browser; on the Android 14 emulator a test APK showed "Hunario" with the new icon in the app list and on the splash screen (test files restored afterwards).
- Tests: `tests/Feature/Admin/AppBrandTest` (8), `resources/js/ui/__tests__/admin-brand.test.js` (4).

### Own TURN server in the admin panel ✅
- **Admin → App settings → Call server (TURN)** (`#turn`): status badge (Not set up / Not checked / Working / Problem), each TURN address with its last check (relay address and time, or what is wrong), **Check from the server** and **Test from this browser**, and the one-line setup command with a Copy button, the hosting-firewall ports and a link to guide step 19. The dashboard's server health shows the call server too.
- **Real check:** `App\Support\Stun\TurnProbe` asks each address for a relay over UDP, TCP or TLS with the app's own short-lived credentials (401 challenge → signed Allocate → Refresh 0 to free it), and tells apart: no answer (coturn down / port closed), wrong secret (not the `static-auth-secret`), quota/capacity, and a working relay. `StunMessage` encodes and verifies STUN/TURN messages; it reproduces the RFC 5769 test vectors byte for byte, and Chrome's WebRTC accepted a relay from the test TURN server built on it. `chat:doctor` uses the same check.
- **Browser check:** `resources/js/ui/admin-turn.js` gathers relay-only ICE candidates with credentials from `admin.turn.servers` (10 minutes, admins only) and explains a wrong secret (401) or no answer; Chrome's normal password challenge (code 1) is ignored.
- **Setup:** `scripts/setup-turn.sh` keeps an existing coturn secret, saves the addresses and secret through `php artisan chat:turn-server --urls=… --secret-stdin` (encrypted in app settings, never on the command line; a fixed username/password is cleared) and runs `chat:turn-server --check` instead of editing `.env`.
- **Review fixes:** the server check is labelled *Answers* (not *Working*): it runs on the server itself, so the hosting provider's firewall is confirmed with *Test from this browser* on a phone (wording in the admin page, dashboard, script and guide); the saved result is dropped when the addresses, secret or password change; a server that relays without a password is a *Problem* (and the relay is freed); the manual coturn config in guide 19.2 matches the script (ports 49160–49400, quota 200, IPv6 private ranges, conf readable only by coturn); the admin check stays inside a 20 second budget.
- Tests: `tests/Unit/StunMessageTest` (4, RFC 5769), `tests/Feature/Admin/TurnServerTest` (7, against `tests/Fixtures/fake-turn-server.php` over UDP and TCP), `resources/js/ui/__tests__/admin-turn.test.js` (5).

### Admin panel UI refresh ✅
- **Sidebar:** tighter rows so every link (Backups, Guides) fits on a laptop screen; a slim footer with *Back to chats* and the audit-log note instead of a boxed note.
- **Top bar:** *Find a person…* search on every admin page (press `/`), a glassy sticky bar, content centred on wide screens.
- **Dashboard:** four headline numbers (users, online now, messages, open reports) with a 30-day trend line (`<x-admin.sparkline>`), open reports highlighted when there are any; the other eight numbers in one compact *at a glance* card (2 columns on phones instead of 12 full-width cards).
- **Person page:** the 16 number tiles became an *In numbers* card grouped into Messages, Chats & groups, Calls, People & devices (each links to its tab); details show labels in one column and values left-aligned; a smaller profile card on phones.
- **App settings:** a section menu (side list on wide screens, sticky chips on phones) that marks the section on screen, in page order with a new *Notice* entry; the save bar only follows you when something changed (*You have unsaved changes*); file pickers styled like the other fields.
- Tab rows fade where more tabs are hidden and scroll the current tab into view.
- Files: `resources/views/components/layouts/admin.blade.php`, `admin/dashboard.blade.php`, `admin/users/tabs/overview.blade.php`, `admin/settings.blade.php`, `components/admin/sparkline.blade.php`, `resources/css/admin.css`, `resources/js/ui/admin-ui.js`. Tests: `tests/Feature/Admin/AdminLayoutTest` (4), `resources/js/ui/__tests__/admin-ui.test.js` (4). Checked in the browser on desktop, phone and dark mode.

### Ads with consent (Y1) — house ads, targeting & personalisation ✅
- **Consent-first:** a one-time "Personalised ads" choice in the app (and a switch in Settings › Privacy). Off by default; declining still shows ads, but generic ones. Turning it off deletes the ad profile immediately. We never read contacts, messages or exact location — only the user's own country (from their number), broad in-app activity segments, device basics (time zone, language, platform), and — only with permission — a coarse (rounded ~1 km) location.
- **Targeting profile** (`AdTargetingService`, table `ad_profiles`): built only for opted-in users. Segments from app activity (business, active, new, groups, callers, media). Optional gender/birth year the user can enter. Country from the phone's dial code (`App\Support\DialCode`).
- **House ads** (Admin → Ads): create sponsored cards (image, headline, text, sponsor, button, link) with a live preview; target by country (everyone — from the number), and by segment/age/gender (opted-in users only); schedule, priority (weighted pick) and a per-person daily cap. A "Sponsored" card appears in the chat list every N chats; impressions and clicks are tracked (`ad_views`, `ad_stats`), with CTR and a per-campaign chart.
- **Serving** (`AdService`): master switch, schedule, targeting, frequency cap and de-dup of a tapped ad; `GET /ads/next` records the view, `GET /ads/{id}/go` records the tap and forwards to the advertiser (opens in the browser, keeping the chat open).
- **Admin:** App settings → Ads (master switch, house-ad frequency, and the AdMob / AdSense fields for the next step); the ad campaigns manager; audit log entries; a warning when ads are off.
- **AdMob (Android) & AdSense (web):** the admin can save the IDs now; wiring the Google SDKs into the app is the next step (needs your AdMob/AdSense account, a CSP allowance and a new APK — see below).
- Files: `app/Models/{AdProfile,AdCampaign,AdView}.php`, `app/Services/{AdService,AdTargetingService}.php`, `app/Support/DialCode.php`, `app/Http/Controllers/AdController.php`, `app/Http/Controllers/Admin/AdController.php`, `resources/js/chat/ads.js`, `resources/js/ui/{admin-ads,ads-settings}.js`, `resources/views/admin/ads/*`. Tests: `tests/Unit/DialCodeTest` (2), `tests/Feature/AdsTest` (11), `tests/Feature/Admin/AdCampaignTest` (7), `resources/js/chat/__tests__/ads.test.js` (5), `resources/js/ui/__tests__/ads-admin.test.js` (3). PHPUnit 533, Vitest 268.

### Ads, second pass — placements, phone permissions, profile details (Y1) ✅
Reworked after review. Ads are the admin's decision, not the user's: there is no longer an on/off switch for ads in the app.
- **Placements** (`AppSupportAdPlacement`): Chats list, Status updates, Channels, Calls and a banner inside an open chat. Each one is switched on or off for the whole app in **App settings → Ads**, and every campaign says which of them it may run in. A screen that is off is never even asked for an ad.
- **Per-placement preview and figures:** the ad editor draws the card the way it appears on each screen (a list row between two chats, or a full-width banner), with a tab per placement, and shows the views and taps that placement has earned (`ad_placement_stats`).
- **Permissions like contacts and camera:** **Location** joined the startup permission sheet and the native plugin (`NativeAppPlugin` alias `location`). The app only ever reads a rounded position once the phone has granted it — it never brings up a prompt of its own. Settings › Privacy › Ads is now an explanation plus that one switch.
- **What we use:** the country of the person's own number, the connection's IP (and its country when the host sends `CF-IPCountry`), the device (platform, version, model), time zone and language, how often the app is opened, what they do in the app, and the **gender and date of birth they set in Settings → Profile**. Never contacts, messages or an exact position.
- **Lazy loading:** an ad is fetched only for the screen the person is actually looking at. Asking for every placement at once spent the whole daily limit on screens they never opened.
- Files: `app/Support/AdPlacement.php`, `app/Services/{AdService,AdTargetingService}.php`, `app/Http/Controllers/AdController.php`, `resources/js/chat/ads.js`, `resources/js/native/permissions.js`, `resources/views/admin/ads/edit.blade.php`. Tests: `AdsTest` (13), `AdminAdCampaignTest` (7), `ads.test.js` (6), `ads-admin.test.js` (3). PHPUnit 537, Vitest 269.
- **Next:** paid plans (admin-built, with the benefits chosen per plan), promote coins with refer-and-earn, and payments (manual approval, Stripe/PayPal, Google Play Billing).

### Paid features (Y2) — plans, coins & promote, refer & earn, verified badge, payments ✅
Designed by a judge panel (docs/MONETISATION-DESIGN.md), built as five slices on a shared foundation. Everything is off until the admin turns on **App settings → Paid features**.
- **Plans:** admin-built, with the benefits ticked per plan (no ads, verified badge, monthly free coins, bigger upload/group/broadcast/storage limits). Benefits are snapshotted onto the subscription; same-plan renewal extends, a different plan queues; hourly expiry, monthly coins, ending reminders; admin grant/end from the person page.
- **Coins:** one wallet row per person, an append-only ledger with a unique cause-derived idempotency key on every row (`CoinService` — row lock, unsigned balances, purchased coins spent before earned ones, proportional refunds, clawback with recorded shortfall, admin adjust/freeze).
- **Promote (TikTok-style):** status, channel, community, business, custom card or website link, each with its own coins-per-1,000-views rate; promotions are `ad_campaigns` rows owned by a user, served through the existing placements, stopped exactly at the view budget by a conditional update inside `AdService::countView()`; review queue with full/unused refunds; in-app taps open the promoted thing without leaving the app.
- **Refer & earn:** code + link, rewarded once when the invited person verifies their number (User::updated hook), caps and void reasons, Earned shown as withdrawable with a "Coming soon" Withdraw button.
- **Verified badge:** coins (or a plan, or an admin), shown next to names everywhere (`nameWithBadge()`, `<x-verified-badge>`).
- **Payments:** manual transfer with screenshot + admin approval, Stripe Checkout + signature-checked webhook, PayPal server-side capture, Google Play Billing verified with the Play Developer API (native `BillingPlugin` + `native/billing.js`; consume only after the server credited; daily voided-purchase sweep). Payment states keep "paid" and "fulfilled" apart so a failed delivery is visible and retryable; every callback is recorded and idempotent. The Android app is offered Play only; the web gets the rest (Play policy).
- **Admin:** Money overview with flags, Plans, Coin packs, Payments (review/approve/reject/refund/retry, private proof viewing), Promotions, Referrals, Subscriptions, person → Money tab, settings section with per-provider Test connection; all audited. `chat:doctor` gained a Paid features section; refund policy page at /refunds; privacy policy updated; account export/deletion cover the new data.
- Tests: PHPUnit 707 (Money/*: ledger, plans, ads-off, promotions, serving, manual/Stripe/PayPal/Play payments, referrals, badge, wallet; Admin/*: plans, payments, promotions, wallet, referrals, paid settings), Vitest 324 (premium, wallet, refer, promote, admin-payments, billing bridge, badge template, promoted cards). Docs: docs/PAID-FEATURES.md, docs/PLAY-BILLING.md.

**Reviewed and hardened.** Five reviewers went through the paid features from different angles (ledger, security, Play policy, promotion serving, regressions) and two independent refuters voted on every finding; 29 survived and were fixed, each with a regression test:
- **Money:** refunding one payment of a renewed plan now takes off exactly that period instead of the whole subscription (`PlanService::removePeriod`); a renewal no longer buries a queued paid plan; partial Stripe/PayPal refunds are recorded instead of clawing back everything; a locally cancelled Stripe session is expired at Stripe and, if it is paid anyway, the money is delivered rather than dropped; approve/reject are locked against each other.
- **Google Play:** verification trusts Google's own productId and orderId, never the client's; Play is only offered when the service account is actually configured; the Android app never shows a web payment method, a web price or a screenshot form (server refuses them too); a permanently refused token is consumed so the item can be bought again; the void sweep no longer skips a failed reversal; the native plugin no longer drops purchases that arrive in the same batch.
- **Promotions:** a promotion whose screens the admin switched off serves nowhere (never everywhere) and is swept, stopped and refunded; banning the owner stops their promotions; a promoted status still respects blocks and the owner's privacy lists; rejected or taken-down cards can no longer redirect; the dead-target link no longer loops.
- **Plans:** the bigger upload, group and broadcast limits are now enforced and advertised from the person's own plan (they were dead), and the subscription lookup no longer runs per row (scoped services + eager-loaded creator).
